...
#'
#' @examples
multiple_imputation <- function(obj, data=NULL, nimp=1) {
  if(is.null(data)){
    X <- obj$y
    data <- X
  }else{
  ...
  for (i in 1:nimp) {
    data_imp[[i]] <- data
    data_imp[[i]]$mm_pattern <- unlist(lapply(1:nrow(data), function(x) sample(1:ng, size = 1, replace = TRUE, prob = pClY[x, ])))
  }
  return(data_imp)
}
