...
#'
#' @description This function compares and plot the accuracy of LCA models with different number of classes for multimorbidity patterns using the misclassification probability and entropy on both the data used to fit the LCA and on a separate dataset. The dashed line corresponds to the assignment accuracy based on the prior.
...
ggaccuracy_LCA <- function(obj, test = NULL) {
  ...
  if (!is.null(test)) {
    dat$ext_accuracy <- unlist(lapply(1:nrow(dat), function(x) sum(diag(get_internal_validation_matrix(obj$obj[[x]], test)))/dat$nclass[x]))
